...
export default function CharacterAnimePage({ params }) {
  const { id } = use(params);
  ...
  const fetchCharacterAnime = async (pageNum = 1, currentSort = sort) => {
    if (pageNum === 1) setInitialLoading(true);
    else setFetchingMore(true);

    try {
      const { data } = await api.get(`/api/anime/character/${id}?page=${pageNum}&limit=24&sort=${currentSort}`);
      if (data.data) {
        setCharacter(data.data.character || null);
      ...
      console.error("Failed to fetch character anime:", err);
    } finally {
  ...
  useEffect(() => {
    setPage(1);
    setAnime([]);
    fetchCharacterAnime(1, sort);
  }, [id, sort]);

  useEffect(() => {
    if (page > 1) {
      fetchCharacterAnime(page, sort);
    }
  }, [page]);
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 pt-8 pb-16">
        ...
        <div className="mb-10 flex flex-col sm:flex-row items-center sm:items-end gap-6">
          <div className="relative h-32 w-32 shrink-0 rounded-full overflow-hidden border-4 border-[var(--border)] bg-[var(--bg-card)] shadow-lg">
            {character?.imageUrl ? (
              <Image src={character.imageUrl} alt={character.name} fill className="object-cover" unoptimized />
            ) : (
          ...
          <div className="text-center sm:text-left">
            ...
            <p className="text-[var(--text-secondary)] text-sm">
              Character · Appears in {meta?.total || 0} anime
            </p>
          </div>
        </div>
        ...
        {meta && (
          <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
            <p className="text-sm font-medium text-[var(--text-secondary)]">
              Showing <span className="text-[var(--text-primary)]">{anime.length}</span> appearances
            </p>
        ...
        {initialLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <h3 className="text-lg font-medium text-[var(--text-primary)]">No appearances found</h3>
            <p className="mt-1 text-sm text-[var(--text-secondary)]">This character has no anime registered yet.</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center gap-2 text-xs text-[var(--text-tertiary)]">
                    ...
                    {item.voiceActor && (
                      <>
                        ...
                        <span className="text-[var(--text-secondary)] font-medium">Voiced by {item.voiceActor.name}</span>
                      </>
                    )}
                  </div>
                </div>
